...
const AgendaView = ({ tweaks, userName, onSwitchService }) => {
  ...
  const countMisTurnos = D.WEEK_DAYS.filter(d => getShifts(d.key).some(s => s.miTurno)).length;
      ...
      <div style={{ display:'flex', gap:6, padding:'10px 14px 6px', overflowX:'auto' }}>
        {FILTERS.map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)} style={{
            background: filter === f.id ? PA.primary : '#fff', color: filter === f.id ? '#fff' : PA.ink2,
            border: `1px solid ${filter === f.id ? PA.primary : PA.line}`, borderRadius: 999, padding: '7px 12px',
            fontSize: 12.5, fontWeight: 800, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap', flexShrink: 0,
          }}>
            {f.label}
            {f.count != null && <span style={{ background: filter === f.id ? 'rgba(255,255,255,0.22)' : PA.line2, color: filter === f.id ? '#fff' : PA.ink3, padding: '1px 6px', borderRadius: 99, fontSize: 10.5, fontWeight: 800 }}>{f.count}</span>}
          </button>
        ))}
      </div>

      <div style={{ flex:1, overflow:'auto', padding:'6px 14px 16px' }}>
        {visibleDays.map(day => <Prop4DayRow key={day.key} day={day} density={tweaks.density} defaultExpanded={day.key === D.TODAY_KEY && filter === 'todos'} onOpen={s => setDetailShift(s)} />)}
        {visibleDays.length === 0 && <div style={{ padding:40, textAlign:'center', color:PA.ink3, fontSize:13, fontWeight:600 }}>Sin turnos en esta categoría.</div>}
      </div>
...
const Prop4DayRow = ({ day, defaultExpanded, onOpen, density }) => {
  ...
  return (
    <div style={{ background: '#fff', border: `1px solid ${hoy ? PA.primary : PA.line}`, borderRadius: 14, marginBottom: 8, overflow: 'hidden', boxShadow: hoy ? '0 4px 14px rgba(23,65,108,0.08)' : 'none' }}>
      <button onClick={() => setExpanded(e => !e)} style={{ width:'100%', background:'transparent', border:'none', padding: vpad, cursor:'pointer', textAlign:'left', display:'flex', alignItems:'center', gap:12 }}>
          ...
          {miShift ? (
            <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
              <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
                <div style={{ padding:'3px 8px', borderRadius:99, background: team.bg, color: team.ink, fontSize:11, fontWeight:800, display:'inline-flex', alignItems:'center', gap:4 }}>
                  <SGTIcon name={miShift.tipo === 'dia' ? 'sun' : 'moon'} size={11} color={team.ink}/> {miShift.tipo === 'dia' ? 'Día' : 'Noche'}
                </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
              ...
              {libres.length > 0 && <span style={{ fontSize:11.5, color:'#B85A60', fontWeight:700 }}>{libres.length} turno libre disponible</span>}
            </div>
          )}
        </div>
        <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:4 }}>
          {libres.length > 0 && miShift && (
            <span style={{ padding:'2px 7px', borderRadius:99, background: PA.accentSoft, color:'#B85A60', fontSize:10, fontWeight:800, border:'1px solid #F3D2D5', display:'inline-flex', alignItems:'center', gap:3 }}>
              <SGTIcon name="hand-raised" size={10} color="#B85A60"/> {libres.length} libre
            </span>
          )}
      ...
      {expanded && (
        <div style={{ padding:'0 14px 12px', borderTop:`1px solid ${PA.line2}` }}>
          {shifts.length === 0 ? <p style={{ fontSize:12, color:PA.ink3, margin:'10px 0 0', fontWeight:600 }}>No hay turnos este día.</p> : shifts.map(s => {
            const t = D.TEAMS[s.equipo];
            return (
              <div key={s.id} onClick={() => onOpen(s)} style={{ marginTop:10, background: t.bg, border:`1px solid ${t.soft}`, borderRadius:10, padding:10, cursor:'pointer' }}>
                <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:7 }}>
                  <SGTIcon name={s.tipo === 'dia' ? 'sun' : 'moon'} size={14} color={t.ink}/>
                  <span style={{ fontSize:12.5, fontWeight:800, color:t.ink, flex:1 }}>{s.tipo === 'dia' ? 'Turno día' : 'Turno noche'} · {s.inicio}–{s.fin}</span>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(255,255,255,0.7)', padding:'6px 8px', borderRadius:8 }}>
                  ...
                  <span style={{ fontSize:11.5, fontWeight:700, color:PA.ink, flex:1, minWidth:0, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{s.team.jefe.nombre.split(' ').slice(0,2).join(' ')}</span>
                  <div style={{ display:'flex' }}>
                    {s.team.urgenciologos && s.team.urgenciologos.slice(0,2).map((p,i) => <div key={p.id} style={{ marginLeft: i===0 ? 0 : -5 }}><SGTAvatar person={p} size={20} ring="#fff"/></div>)}
                  </div>
                  <span style={{ fontSize:10.5, color:PA.ink3, fontWeight:700 }}>+{Math.max(0, s.team.total - 3)} · Ver detalle →</span>
                </div>
              </div>
            );
          })}
...
const TeamComposition = ({ team, density = 'cozy' }) => {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: density === 'compact' ? '8px 10px' : '10px 12px', background: P2().surface2, borderRadius: 10, border: `1px solid ${P2().line}` }}>
        <SGTAvatar person={team.jefe} size={32} ring="#E9D9C2"/>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13.5, fontWeight: 800, color: P2().ink }}>{team.jefe.nombre}</div>
      ...
      {expanded && (
        <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <RoleGroup title="Urgenciólogos" role="URGENCIOLOGO" people={team.urgenciologos || []} ideal={4}/>
          <RoleGroup title="Médicos generales" role="MEDICO" people={team.medicos || []} ideal={4}/>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ border: `1px solid ${P2().line}`, borderRadius: 10, background: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', borderBottom: `1px solid ${P2().line2}` }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><SGTRoleChip role={role} /><span style={{ fontSize: 12, color: P2().ink2, fontWeight: 700 }}>{people.length}/{ideal}</span></div>
      </div>
      <div style={{ padding: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
        {people.map(p => (
          <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 6px', background: p.esYo ? P2().primarySoft : 'transparent', borderRadius: 6 }}>
            <SGTAvatar person={p} size={24}/>
            <span style={{ fontSize: 13, color: P2().ink, fontWeight: p.esYo ? 800 : 500, flex: 1 }}>{p.nombre}{p.esYo ? ' (tú)' : ''}</span>
          </div>
        ))}
      </div>
    </div>
...
const ActionBtn = ({ label, icon, tone = 'primary' }) => {
  ...
  return (
    <button style={{ display: 'flex', alignItems: 'center', gap: 10, background: t.bg, color: t.ink, border: `1px solid ${t.bd}`, padding: '12px 14px', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', textAlign: 'left' }}>
      <SGTIcon name={icon} size={17} color={t.ink}/><span style={{ flex: 1 }}>{label}</span><SGTIcon name="chevron-right" size={14} color={t.ink} strokeWidth={2.4}/>
    </button>
  );
};
...
const ShiftDetail = ({ shift }) => {
  ...
  return (
    <div style={{ padding: '0 16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ background: team.bg, border: `1px solid ${team.soft}`, borderRadius: 14, padding: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}><SGTIcon name={shift.tipo === 'dia' ? 'sun' : 'moon'} size={18} color={team.ink}/><span style={{ fontSize: 13, fontWeight: 800, color: team.ink, textTransform: 'uppercase' }}>{shift.tipo === 'dia' ? 'Turno día' : 'Turno noche'}</span></div>
        <div style={{ fontSize: 22, fontWeight: 800, color: team.ink }}>{shift.inicio} – {shift.fin}</div>
      </div>
      <div>
        <div style={{ fontSize: 12, fontWeight: 800, color: P2().ink3, textTransform: 'uppercase', marginBottom: 8 }}>Composición del turno</div>
        <TeamComposition team={shift.team} />
      </div>
      <div>
        <div style={{ fontSize: 12, fontWeight: 800, color: P2().ink3, textTransform: 'uppercase', marginBottom: 8 }}>Acciones</div>
